        ...
        <div class="col-xl-5 offset-xl-3">
            <div class="login-register-page">
                ...
                <form method="post" id="register-account-form" action="#" accept-charset="UTF-8" onsubmit="document.getElementById('submit-btn').disabled = true;">
                    ...
                    <div class="form-group">
                        <div class="input-with-icon-left">
                            <i class="country-number"><small>+62</small></i>
                            <input type="number" class="input-text with-border" placeholder="<?php _e("Whatsapp Number") ?>" value="<?php _esc($nomor_wa)?>" id="no_wa" name="no_wa" onBlur="checkAvailabilityPhone()" required/>
                        </div>
                        <span id="wa-availability-status"><?php if($phone_error != ""){ _esc($phone_error) ; }?></span>
                    </div>
                    <div class="form-group">
                        <div class="input-with-icon-left">
...
<script>
    ...
    function checkAvailabilityPhone() {
        ...
        jQuery.ajax({
            ...
            data: 'no_wa=' + $("#no_wa").val(),
            type: "POST",
            success: function (data) {
                if (data != "success") {
                    error = 1;
                    $("#no_wa").removeClass('has-success');
                    $("#wa-availability-status").html(data);
                    $("#no_wa").addClass('has-error mar-zero');
                }
                else {
                    error = 0;
                    $("#no_wa").removeClass('has-error mar-zero');
                    $("#wa-availability-status").html("");
                    $("#no_wa").addClass('has-success');
                }
                $("#loaderIcon").hide();
            },
            error: function () {
