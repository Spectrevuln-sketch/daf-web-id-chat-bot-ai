...
if(checkloggedin())
{
    ...
    if(isset($_POST['submit']))
    {
        ...
        // Check if this is an Phone availability check from signup page using ajax
        if(is_null($_POST["phone"])) {
            $errors++;
            $email_error = __("Please enter an phone address");
            $phone_error = "<span class='status-not-available'> ".$phone_error."</span>";
        }
        elseif($_POST["phone"] != $ses_userdata['phone'])
        {

                $user_count = check_nomor_wa($_POST["phone"]);
                if ($user_count > 0) {
                    $errors++;
        ...
        if($errors == 0)
        {
            ...
            $phone ='';
            if(preg_match('/^[0]/', $_POST["no_wa"]) == false){
                if(preg_match('/^(62)/', $_POST["no_wa"])){
                    $phone = preg_replace('/^(62)/','0', $_POST["no_wa"]);
                }else{
                    $phone =  '0'.$_POST["no_wa"];
                }
            }else{
