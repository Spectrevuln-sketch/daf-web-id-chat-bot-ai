...
function delete_otp($phone){
    $current_otp = ORM::for_table($config['db']['pre'].'otp')->where('phone', $phone)->find_many();
...
function create_otp_session($phone, $expired, $userid, $username, $password, $user_type = ''){
    $user_browser = $_SERVER['HTTP_USER_AGENT'];
    $_SESSION['otp']['phone'] =$phone;
    $_SESSION['otp']['expired']=$expired;
    $user_id = preg_replace("/[^0-9]+/", "", $userid); // XSS protection as we might print this value
    $_SESSION['otp']['id'] = $user_id;

    $username = preg_replace("/[^a-zA-Z0-9_\-]+/", "", $username); // XSS protection as we might print this value
    $_SESSION['otp']['username'] = $username;
    $_SESSION['otp']['user_type'] = $user_type;

    $_SESSION['otp']['password'] = $password;
}
 ...
 */
function create_user_session($userid, $username, $phone,  $password, $user_type = '')
{
...
function userlogin($email, $password)
{
    ...
    if ($num_rows >= 1) {
        ...
        if (password_verify($password, $db_password)) {
            ...
            // $userinfo
            // $wa_sender = consume_third_party('http://116.203.191.58/api/async_send_message', 'POST', $userinfo);
            return $userinfo;
 ...
 * @param int $id
 */
function send_forgot_wa($user, $message=null)
{
    global $config, $lang, $link;
    $key='23bbc54a27dc91438a6f4b615a37754e9f85980d7ae241de';
        $phone_number = str_replace('-','', $user['phone']);
        ...
        $payload= [
            ...
            "key"       => $key,
            "message"   =>  $message == null ? 'Masukan kode otp berikut *'.$otp_code.'*' : $message,
            "skip_link" => True //
        ];
 ...
 * @param int $id
 */
function send_wa_message($user, $message=null)
{
    global $config, $lang, $link;
    $key='23bbc54a27dc91438a6f4b615a37754e9f85980d7ae241de';
        $phone_number = str_replace('-','', $user['phone']);
