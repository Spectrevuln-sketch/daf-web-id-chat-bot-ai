...
if(isset($_POST['send'])){
  ...
  foreach($_POST as $key=> $post){
    ...
    if($post == ''){
      HtmlTemplate::display('global/otp',[
        'error'=> __("Must input 6 digits of OTP")
        ]);
    }
  }
  if(count($otp) < 6){
  ...
  if(empty(find_user['id'])){
    ...
    create_user_session($_SESSION['otp']['id'],$_SESSION['otp']['username'],$_SESSION['otp']['password'],$_SESSION['otp']['user_type']);
    update_lastactive();
