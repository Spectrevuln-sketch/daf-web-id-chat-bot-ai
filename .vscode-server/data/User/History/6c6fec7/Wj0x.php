...
else
{
    ...
    $payload= [
        "phone_no"  => '+'. $user_current['phone'],
        "key"       => $key,
        "message"   => 'Masukan kode otp berikut',
        "skip_link" => True //
    ];
    $waSend = consume_third_party('http://116.203.191.58/api/send_message', 'POST', $payload);
    $insert_otp = ORM::for_table($config['db']['pre'].'otp')->create();
    $insert_otp->phone= $user_current['phone'];
    $insert_otp->otp_num= random_int(100000, 999999);
    $insert_otp->expired= date('Y-m-d', strtotime('+2 minutes'));
