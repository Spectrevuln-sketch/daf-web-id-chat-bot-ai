...
if(isset($_POST['phone'])){
    if(empty($_POST["phone"])) {
        $name_error = __('Enter your phone number.');
        echo "<span class='status-not-available'> ".$name_error."</span>";
        exit;
    }
    if(checkloggedin()){
        if($_POST["phone"] != $_SESSION['user']['phone']){
            $phone_count = check_nomor_wa($_POST['phone']);
            if($phone_count>0) {
                $phone_error = __('Phone number not available');
                echo "<span class='status-not-available'>".$phone_error."</span>";
            }
            else {
                $phone_error = __("Phone number available");
...
if(isset($_POST['no_wa'])){
    ...
    if(checkloggedin()){
        if($_POST["no_wa"] != $_SESSION['user']['no_wa']){
            $no_wa_count = check_nomor_wa($_POST['no_wa']);
            if($no_wa_count>0) {
                $no_wa_error = __('Phone number not available');
                echo "<span class='status-not-available'>".$no_wa_error."</span>";
            }
            else {
                $no_wa_error = __("Phone number available");
                echo "<span class='status-available'>".$no_wa_error."</span>";
            }
            exit;
        }else{
            echo "<span class='status-available'>".__('Success')."</span>";
